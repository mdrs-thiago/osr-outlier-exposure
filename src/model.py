...
from generator import DataGenerator, DataGeneratorWithCoords, DataGeneratorWithCoordsRandom, DataGeneratorOOD
...
#		super().__init__(*args, **kwargs)
		...
		self.save_model_name = paramsTrain.save_model_name
...
class ModelFit(NetModel):
	def train(self, data, ood=None):
		...
		ic(self.name)
		history = self.applyFitMethod(data, ood=ood)
		...
		self.graph.save(self.save_model_name)
		ic(self.name)
...
class ModelLoadGeneratorWithCoords(ModelFit):
	...
	def applyFitMethod(self,data, ood = None):
		ic(self.class_n)
		#pdb.set_trace()
		params_train = {
...
#			'printCoords': False,
		...
		self.paramsTrain.trainOOD = True
		...
		if generator_type=="coords":
			...
			elif self.paramsTrain.trainOOD == True:
				training_generator = DataGeneratorOOD(data.full_ims_train, data.full_label_train, 
					data.patches['train']['coords'], ood.full_ims_train, ood.full_label_train, samples_per_epoch = 9100, **params_train)
			else:
...
#		pdb.set_trace()
		print('&&'*100)
		print(f'n classes = {self.class_n}')
...
#				validation=(data.patches['val']['in'], data.patches['val']['label']),				
				...
				patience=15, classes=self.class_n)], # it was 5
			shuffle = False
			)
...
#		layer_names = ['conv_lst_m2d_1', 'activation_6', 'activation_8', 'activation_10']
		...
		all_layer_names=[layer.name for layer in self.graph.layers]
		print(all_layer_names)
		indexes = [16, 24, 32, 40]
		layer_names = [layer for j, layer in enumerate(all_layer_names) if j in indexes]
		print(layer_names)
		...
		#out1 = UpSampling2D(size=(upsample_ratios[0], upsample_ratios[0]))(self.graph.layer[16].output)
		#out2 = UpSampling2D(size=(upsample_ratios[1], upsample_ratios[1]))(self.graph.layer[24].output)
		#out3 = UpSampling2D(size=(upsample_ratios[2], upsample_ratios[2]))(self.graph.layer[32].output)
		#out4 = UpSampling2D(size=(upsample_ratios[3], upsample_ratios[3]))(self.graph.layer[40].output)
