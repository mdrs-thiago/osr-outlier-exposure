...
#		ic(os.listdir(self.conf["train"]["mask"]["dir"]))
		...
		np.save(self.paramsTrain.path / 'full_ims/full_ims_test_{}.npy'.format(self.dataset.seq_date),self.full_ims_test.astype(np.float16))
		np.save(self.paramsTrain.path / 'full_ims/full_ims_train_{}.npy'.format(self.dataset.seq_date),self.full_ims_train.astype(np.float16))
		np.save(self.paramsTrain.path / 'full_ims/full_label_test_{}.npy'.format(self.dataset.seq_date),self.full_label_test)
		np.save(self.paramsTrain.path / 'full_ims/full_label_train_{}.npy'.format(self.dataset.seq_date),self.full_label_train)


#		np.save(self.paramsTrain.path / 'full_ims/full_label.npy',self.full_label)

	def fullImsLoad(self):
		self.full_ims_test = np.load(self.paramsTrain.path / 'full_ims/full_ims_test_{}.npy'.format(self.dataset.seq_date))
		self.full_ims_train = np.load(self.paramsTrain.path / 'full_ims/full_ims_train_{}.npy'.format(self.dataset.seq_date))
		self.full_label_test = np.load(self.paramsTrain.path / 'full_ims/full_label_test_{}.npy'.format(self.dataset.seq_date))
		self.full_label_train = np.load(self.paramsTrain.path / 'full_ims/full_label_train_{}.npy'.format(self.dataset.seq_date))
...
#		deb.prints(len(gridy))
		...
		np.save(self.paramsTrain.path / "coords_train_{}.npy".format(self.dataset.seq_date), coords_train) # to-do: create coords folder
		np.save(self.paramsTrain.path / "coords_test_{}.npy".format(self.dataset.seq_date), coords_test)
