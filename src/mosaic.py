...
#		deb.prints(direct_execution)
	...
	def defineMosaicVars(self, h, w, class_n):
		...
		self.h = h
		self.w = w
	def create(self, paramsTrain, model, data, ds, postProcessing = None, KUC = None):
		self.KUC = KUC
		self.model = model
		self.data = data
		self.ds = ds
...
class MosaicHighRAMPostProcessing(MosaicHighRAM):
	def create(self, paramsTrain, model, data, ds, postProcessing, KUC=None):
		self.postProcessing = postProcessing
			#pdb.set_trace()

		super().create(paramsTrain, model, data, ds, KUC=KUC)
