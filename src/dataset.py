...
#				deb.prints(dotys_sin_cos.shape)
	...
	def reloadLabel(self, train=False):
		self.full_label_test = np.load(self.path['v'] / 'full_ims' / 'full_label_test_{}.npy'.format(self.paramsTrain.seq_date)).astype(np.uint8)
		self.full_label_test = self.full_label_test[-1]

		if train == True:
			self.full_label_train = np.load(self.path['v'] / 'full_ims' / 'full_label_train_{}.npy'.format(self.paramsTrain.seq_date)).astype(np.uint8)
			self.full_label_train = self.full_label_train[-1]


	def getTrainLabelWithUnknown(self):
		label = np.load(self.path['v'] / 'full_ims' / 'full_label_train_{}.npy'.format(self.paramsTrain.seq_date)).astype(np.uint8)[-1]
...
#		super().create_load()
		...
		self.patches['train']['coords'] = np.load(self.path['v'] / 'coords_train_{}.npy'.format(self.paramsTrain.seq_date)).astype(np.int)
		self.patches['test']['coords'] = np.load(self.path['v'] / 'coords_test_{}.npy'.format(self.paramsTrain.seq_date)).astype(np.int)
		ic(self.patches['train']['coords'].shape)
		#pdb.set_trace()
		self.full_ims_train = np.load(self.path['v'] / 'full_ims' / 'full_ims_train_{}.npy'.format(self.paramsTrain.seq_date))
		self.full_ims_test = np.load(self.path['v'] / 'full_ims' / 'full_ims_test_{}.npy'.format(self.paramsTrain.seq_date))
...
#		pdb.set_trace()
		...
		if self.paramsTrain.support_set is not None:
			self.paramsTrain.known_classes = self.paramsTrain.support_set
			...
			self.paramsTrain.UUC = self.paramsTrain.query_set

			deb.prints(self.paramsTrain.support_set)
			#pdb.set_trace()

		elif self.paramsTrain.selectMainClasses==True:
			...
			deb.prints(all_classes)
			#deb.prints(self.paramsTrain.known_classes)
			known_classes = np.setdiff1d(all_classes, self.unknown_classes)
			self.paramsTrain.known_classes = known_classes[:5]
			self.paramsTrain.UUC = known_classes[5:]
			deb.prints(self.unknown_classes)
...
##		deb.prints(np.unique(self.patches['train']['label'].argmax(axis=-1),return_counts=True))
	...
	def small_classes_ignore(self, label, predictions, important_classes_idx, KUC = None):
		class_n = 15
		important_classes_idx = list(important_classes_idx)
		ic(important_classes_idx)
		important_classes_idx.append(255) # bcknd
		#important_classes_idx.append(39) #bcknd for prediction mask
		#predictions[label==255] = 255
		...
		for idx in range(class_n):
			if KUC is not None:
				if idx in KUC:
					#predictions[predictions==idx]=255
					label[label==idx]=255

				if (idx not in important_classes_idx) and (idx not in KUC):
