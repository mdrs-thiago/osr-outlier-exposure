...
class DataGeneratorWithCoords(keras.utils.Sequence):
	...
	def __init__(self, inputs, labels, coords, samples_per_epoch = None,
				batch_size=16, dim=(20,128,128), label_dim=(128,128),
...
class DataGeneratorOOD(keras.utils.Sequence):
	'Generates data for Keras'
	def __init__(self, inputs, labels, coords, ood, ood_labels, samples_per_epoch = None,
				batch_size=28, dim=(20,128,128), label_dim=(128,128),
				n_channels=3, n_classes=2, shuffle=False, center_pixel = False, printCoords=False,
				augm = False):
		'Initialization'
		self.inputs = inputs
		...
		self.labels = labels
		self.background_ood = np.max(ood_labels)
		self.n_channels = n_channels
		self.n_classes = n_classes
		self.shuffle = shuffle
		self.label_dim = label_dim
...
#		self.indexes = np.arange(len(self.list_IDs))
	...
	def __data_generation(self, coords_batch):
		...
		'Generates data containing batch_size samples' # X : (n_samples, *dim, n_channels)
		# Initialization
		X = np.empty((self.batch_size + 4, *self.dim, self.n_channels), dtype=np.float32)
#		Y = np.empty((self.batch_size, *self.label_dim, self.n_classes), dtype=int)
		Y = np.empty((self.batch_size + 4, *self.label_dim), dtype=int)

		background = self.n_classes	#self.n_classes já usa quantidade de classes +1
		#deb.prints(coords_batch)
		#deb.prints(coords_batch.shape[0])
		if self.printCoords:
			ic(coords_batch)
		for idx in range(coords_batch.shape[0]):
			...
			if self.center_pixel == False:
				...
				if idx < 4:
					...
					ood_patch = self.ood_data[:, coords_batch[idx][0]:coords_batch[idx][0]+self.patch_size,
										coords_batch[idx][1]:coords_batch[idx][1]+self.patch_size]

					
					ood_labels = self.ood_labels[coords_batch[idx][0]:coords_batch[idx][0]+self.patch_size,
						coords_batch[idx][1]:coords_batch[idx][1]+self.patch_size]

					
					#uniques = np.unique(self.ood_labels)
					#i_un = np.unique(label_patch)

					#print(f'n classes = {self.n_classes}')
					#print('OOD labels')
					#print(self.background_ood)
					#print(np.unique(ood_labels, return_counts = True))
					ood_labels = np.where(ood_labels == self.background_ood, background,background + 1)
					#ood_labels[ood_labels!=uniques[-1]] = uniques[-1] - 1
					#ood_labels[ood_labels==uniques[-1]] = i_un[-1]
					#print('Self labels')
					#print(np.unique(ood_labels, return_counts=True))
					#print(uniques)
					#print('-------')
					#pdb.set_trace()
					'''
					ood_labels = -1*np.ones_like(label_patch)
					'''
...
#			Y[idx] = toOneHot(label_patch)
			Y[idx] = label_patch
			if idx < 4:
				X[self.batch_size + idx] = ood_patch
				Y[self.batch_size + idx] = ood_labels
...
class DataGeneratorWithCoordsRandom(DataGeneratorWithCoords):
	def on_epoch_end(self):
		'Updates indexes after each epoch'
#		self.indexes = np.arange(len(self.list_IDs))
		self.indexes = np.arange(self.samples_per_epoch)
		if self.shuffle == True:
			np.random.shuffle(self.indexes)
...
#		return int(np.floor(len(self.list_IDs) / self.batch_size))
#		n_batches = int(np.floor(self.coords.shape[0] / self.batch_size))
		n_batches = int(np.floor(self.samples_per_epoch / self.batch_size))
		ic(n_batches)
		return n_batches
