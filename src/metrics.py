...
#		metrics['recall_avg'] = np.average(recall[:-1])
	...
	def plotROCCurve(self, y_test, y_pred, modelId, nameId, unknown_class_id = 39, known_class_id = None):
		...
		print('TESTING')
		print(np.unique(y_test[y_pred < 0.3], return_counts = True))
		print(np.unique(y_test[y_pred < 0.5], return_counts = True))
		print(np.unique(y_test[y_pred < 0.8], return_counts = True))
		
		#pdb.set_trace()

		if known_class_id:
			y_test[np.isin(y_test,known_class_id)] = 0

			print(f'UNIQUE VALUES FOR ROC CURVE{np.unique(y_test, return_counts=True)}')
			#pdb.set_trace()
			y_test[y_test==unknown_class_id] = 1
			#KKC_UUC = np.concatenate(known_class_id, unknown_class_id)
			y_pred = y_pred[np.isin(y_test,np.array([0,1]))]
		...
		print('TESTING')
		print(np.unique(y_test[y_pred < 0.3], return_counts = True))
		print(np.unique(y_test[y_pred < 0.4], return_counts = True))
		print(np.unique(y_test[y_pred < 0.5], return_counts = True))
		print(np.unique(y_test[y_pred < 0.8], return_counts = True))
...
#        plt.plot(tpr, fpr, label = 'AUC = %0.2f' % roc_auc)
		...
		plt.title(f'ROC Curve - AUC = {np.round(roc_auc,2)}')
...
#        plt.gca().set_aspect('equal', adjustable='box')
		...
		#return roc_auc, thresholds_by_tpr
